...
def uncertainty_sampling(
    ...
    prob_fn = train_LSTM.get_probs if is_seq else train_NN.get_probs

    if trainWL is not None:
        if sampleWL and perfWL:
            NN, best_ap = train_fn((trainX[sel_inds], trainWL[sel_inds], trainY[sel_inds]),
            ...
            NN, _ = train_fn((trainX[sel_inds], trainWL[sel_inds], trainY[sel_inds]),
                             (validX, validWL, validY), (testX, testWL, testY),
                             ap_score,
                             device,
                             no_weight_loss=no_weight_loss,
                             best_map=best_map)
            _, best_ap = train_fn((trainX[sel_inds], None, trainY[sel_inds]),
            ...
            _, best_ap = train_fn((trainX[sel_inds], trainWL[sel_inds], trainY[sel_inds]),
                                  (validX, validWL, validY), (testX, testWL, testY),
                                  ap_score,
                                  device,
                                  no_weight_loss=no_weight_loss,
                                  best_map=best_map)
            NN, _ = train_fn((trainX[sel_inds], None, trainY[sel_inds]), (validX, None, validY),
                             (testX, None, testY),
                             ap_score,
                             device,
                             no_weight_loss=no_weight_loss,
            ...
            NN, best_ap = train_fn((trainX[sel_inds], None, trainY[sel_inds]),
                                   (validX, None, validY), (testX, None, testY),
                                   ap_score,
                                   device,
                                   no_weight_loss=no_weight_loss,
        ...
        NN, best_ap = train_fn((trainX[sel_inds], None, trainY[sel_inds]), (validX, None, validY),
                               (testX, None, testY),
                               ap_score,
                               device,
                               no_weight_loss=no_weight_loss,
    ...
    if len(remain_inds) > 0:
        if trainWL is not None and sampleWL:
            probs = prob_fn(NN, trainX[remain_inds], trainWL[remain_inds], device)
        else:
            probs = prob_fn(NN, trainX[remain_inds], None, device)
...
def random_sampling(trainX,
    ...
    if trainWL is not None and perfWL:
        ...
        _, best_ap = train_fn((trainX[sel_inds], None, trainY[sel_inds]), (validX, None, validY),
                              (testX, None, testY),
                              ap_score,
                              device,
                              no_weight_loss=no_weight_loss,
...
def disagreement_sampling(
    ...
    if perfWL:
        NN, best_ap = train_fn((trainX[sel_inds], trainWL_concat[sel_inds], trainY[sel_inds]),
    ...
    NN_probs = prob_fn(NN, trainX[remain_inds], trainWL_concat[remain_inds], device)
