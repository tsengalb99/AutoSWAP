...
class ProgramGraph(object):
    def __init__(self,
        ...
        start = dsl.StartFunction(input_type=input_type,
    ...
    def construct_candidates(self, input_type, output_type, input_size, output_size, num_units):
        ...
        for functionclass in replacement_candidates:
            if issubclass(functionclass, dsl.ITE):
                candidate = functionclass(input_type,
                                          output_type,
                                          input_size,
                                          output_size,
                                          num_units,
            ...
            for submod, functionclass in current.submodules.items():
                if issubclass(type(functionclass), dsl.HeuristicNeuralFunction):
                    replacement_candidates = self.construct_candidates(
                        ...
                        functionclass.input_size, functionclass.output_size, child_num_units)
                    ...
                    for child_candidate in replacement_candidates:
                        ...
                        if child_node.depth + self.min_depth2go(
                                child_node.program) > self.max_depth:
                            continue
                        # if yes, compute costs and add to list of children
                        child_node.cost = current_node.cost + self.compute_edge_cost(
                            child_candidate)
