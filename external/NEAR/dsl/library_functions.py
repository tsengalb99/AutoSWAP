...
class StartFunction(LibraryFunction):
    def __init__(self, input_type, output_type, input_size, output_size, num_units):
        self.program = init_neural_function(input_type, output_type, input_size, output_size,
                                            num_units)
...
class FoldFunction(LibraryFunction):
    def __init__(self, input_size, output_size, num_units, fold_function=None):
        ...
        if fold_function is None:
            fold_function = init_neural_function("atom", "atom", input_size + output_size,
    ...
    def execute_on_batch(self, batch, batch_lens=None, is_sequential=False):
        ...
        idx = torch.tensor(batch_lens).to(device, non_blocking=True) - 1
        ...
        for t in range(seq_len):
            features = batch[t]
            out_val = self.submodules["foldfunction"].execute_on_batch(
                torch.cat([features, folded_val], dim=1))
...
class MapPrefixesFunction(LibraryFunction):
    ...
    def execute_on_batch(self, batch, batch_lens):
        assert len(batch.size()) == 3
        map_output = self.submodules["mapfunction"].execute_on_batch(batch,
                                                                     batch_lens,
                                                                     is_sequential=True)
...
class ITE(LibraryFunction):
    ...
    def __init__(self,
        ...
        if eval_function is None:
            if simple:
                ...
                eval_function = init_neural_function(input_type, "atom", input_size, output_size,
                                                     num_units)
        if function1 is None:
            function1 = init_neural_function(input_type, output_type, input_size, output_size,
                                             num_units)
        if function2 is None:
            function2 = init_neural_function(input_type, output_type, input_size, output_size,
                                             num_units)
    ...
    def execute_on_batch(self, batch, batch_lens=None, is_sequential=False):
        ...
        if is_sequential:
            predicted_eval = self.submodules["evalfunction"].execute_on_batch(batch,
                                                                              batch_lens,
                                                                              is_sequential=False)
            predicted_function1 = self.submodules["function1"].execute_on_batch(
                batch, batch_lens, is_sequential=is_sequential)
            predicted_function2 = self.submodules["function2"].execute_on_batch(
                batch, batch_lens, is_sequential=is_sequential)
        else:
...
class OuterMultiplyFunction(LibraryFunction):
    def __init__(self, input_size, output_size, num_units, function1=None, function2=None):
        if function1 is None:
            function1 = init_neural_function("atom", "atom", input_size, output_size, num_units)
        if function2 is None:
    ...
    def init_params(self):
        self.linear_layer = nn.Linear(self.output_size**2, self.output_size, bias=True).to(device)
        self.parameters = {"weights": self.linear_layer.weight, "bias": self.linear_layer.bias}
    ...
    def execute_on_batch(self, batch, batch_lens=None):
        ...
        batch_outer_prod = torch.bmm(predicted_function1.unsqueeze(2),
                                     predicted_function2.unsqueeze(1))
        batch_outer_prod = torch.reshape(batch_outer_prod, [batch.size()[0], -1])
        return self.linear_layer(batch_outer_prod)
...
class AppendFunction(LibraryFunction):
    def __init__(self, input_size, output_size, num_units, function1=None, function2=None):
        if function1 is None:
            function1 = init_neural_function("atom", "atom", input_size, output_size, num_units)
        if function2 is None:
    ...
    def execute_on_batch(self, batch, batch_lens=None):
        ...
        concat = torch.cat([predicted_function1, predicted_function2], -1)
        linear_out = self.linear_layer(concat)
        return linear_out
...
class LearnedConstantFunction(LibraryFunction):
    def __init__(self, input_size, output_size, num_units):
        super().__init__({},
                         ...
                         output_size,
                         num_units,
                         name="LearnedConstant",
                         has_params=True)

    def init_params(self):
        self.parameters = {
            "constant": torch.rand(self.output_size, requires_grad=True, device=device)
        }
...
class AffineFunction(LibraryFunction):
    ...
    def init_params(self):
        self.linear_layer = nn.Linear(self.selected_input_size, self.output_size,
                                      bias=True).to(device)
        self.parameters = {"weights": self.linear_layer.weight, "bias": self.linear_layer.bias}
...
class AffineFeatureSelectionFunction(AffineFunction):
    def __init__(self, input_size, output_size, num_units, name="AffineFeatureSelection"):
        ...
        self.feature_tensor = self.feature_tensor.to(device, non_blocking=True)
        super().__init__(raw_input_size=input_size,
                         selected_input_size=self.feature_tensor.size()[-1] + additional_inputs,
    ...
    def init_params(self):
        ...
        if self.is_full:
            ...
            self.feature_tensor = torch.arange(self.input_size).to(device, non_blocking=True)
        ...
        self.linear_layer = nn.Linear(self.selected_input_size, self.output_size,
                                      bias=True).to(device, non_blocking=True)
        self.parameters = {"weights": self.linear_layer.weight, "bias": self.linear_layer.bias}
